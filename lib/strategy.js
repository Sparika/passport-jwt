...
JwtStrategy.prototype.authenticate = function(req, options) {
    ...
    if(self._keyInHeader){
        ...
        //TODO Get JKU from token header
        console.log(token)
        var jkuD = 'https://192.168.99.100:8080'
        var jkuPath = 'https://192.168.99.100:8080/proxy/keyset'

        console.log('Going to request')

        request(jkuPath, function(error, response, body) {
            console.log('Error: '+error)
            console.log('Response: '+response)
            console.log('Body:'+body);

            // Check JKU Domain = ISS Domain
            if(self._verifOpts.issuer != jkuD){
                return self.fail(new Error('JKU Domain and Issue do not match'))
            }

            // TODO Generate key from JWK
