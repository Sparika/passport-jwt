...
JwtStrategy.prototype.authenticate = function(req, options) {
    ...
    if(self._keyInHeader){
        ...
        var jku = header.jku
        if(!jku){
            console.error('No JKU found in token')
            return self.fail(new Error('No JKU found in token'))
        }
        if(!jku.startsWith('https')){
            console.error('JKU is unsecure, no HTTPS')
            return self.fail(new Error('JKU is unsecure, no HTTPS'))
        }

        //TODO Key should be in .well-known
        var jkuD = jku.split('/')[2]

        var iss = JSON.parse(atob(token.split('.')[1])).iss
        // Check JKU Domain = ISS Domain
        if(!iss || !iss.startsWith(jkuD)){
            console.error('JKU Domain and Issuer do not match')
            console.error('jku: '+jku)
            console.error('iss: '+iss)
            return self.fail(new Error('JKU Domain and Issuer do not match'))
        }
